...
/* ✅ FORCE LOGIN PAGE FIRST (ORDER MATTERS) */
app.get('/', (req, res) => {
...
io.on('connection', (socket) => {
  ...
  socket.on('private accept', (data) => {
    const targetSocket = [...io.sockets.sockets.values()]
      .find(s => s.userID == data.fromID);
    if (targetSocket) {
      [socket, targetSocket].forEach(s =>
        s.emit('private accepted', {
          user1: socket.userID,
          user2: targetSocket.userID
        })
      );
    }
  });
